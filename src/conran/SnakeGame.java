...
public class SnakeGame extends JFrame implements Runnable, KeyListener {
    ...
    public SnakeGame() {
        ...
        getContentPane().setPreferredSize(new Dimension(mMainFrameSizeWidth, mMainFrameSizeHeight));
        pack();
    ...
    private void createNewApple() {
        ...
        do {
            ...
            _y = (((int) rand.nextInt(mMainFrameSizeHeight)) / this.mSnakePortionSize) * this.mSnakePortionSize + sExtH + SnakeGame.GAME_BORDER;
            if (_y < sExtH + 2 * SnakeGame.GAME_BORDER || _y > mMainFrameSizeHeight - 2 * SnakeGame.GAME_BORDER) {
                _y = sExtH + SnakeGame.GAME_BORDER * 2;
            }
            mApple.setX(_x);
            mApple.setY(_y);
